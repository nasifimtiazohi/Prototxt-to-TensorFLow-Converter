...
def InceptionV1(inputs,num_classes=1000,is_training=true,reuse=true,scope=InceptionV1,config=None):
	...
	with tf.variable_scope(scope,"Model",reuse=reuse):
		with slim.arg_scope(default_arg_scope(is_training):
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_3b', end_points['MaxPool_3a_3x3'])
			############## end template code ##############
			with tf.variable_scope(Mixed_3b):
				...
				with tf.variable_scope(branch_1):
					...
					branch_1=slim.conv2d(net,selectdepth('Mixed_3b',96),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_3b',16),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_3c', end_points['Mixed_3b'])
			############## end template code ##############
			with tf.variable_scope(Mixed_3c):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_3c',128),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_3c',32),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_4b', end_points['MaxPool_4a_3x3'])
			############## end template code ##############
			with tf.variable_scope(Mixed_4b):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_4b',96),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_4b',16),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_4c', end_points['Mixed_4b'])
			############## end template code ##############
			with tf.variable_scope(Mixed_4c):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_4c',112),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_4c',24),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_4d', end_points['Mixed_4c'])
			############## end template code ##############
			with tf.variable_scope(Mixed_4d):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_4d',128),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_4d',24),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_4e', end_points['Mixed_4d'])
			############## end template code ##############
			with tf.variable_scope(Mixed_4e):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_4e',144),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_4e',32),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_4f', end_points['Mixed_4e'])
			############## end template code ##############
			with tf.variable_scope(Mixed_4f):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_4f',160),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_4f',32),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_5b', end_points['MaxPool_5a_2x2'])
			############## end template code ##############
			with tf.variable_scope(Mixed_5b):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_5b',160),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_5b',32),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			############## template code added for multiplexing ##############
			net = selectinput('Mixed_5c', end_points['Mixed_5b'])
			############## end template code ##############
			with tf.variable_scope(Mixed_5c):
				...
				with tf.variable_scope(branch_1):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_1=slim.conv2d(net,selectdepth('Mixed_5c',192),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
				...
				with tf.variable_scope(branch_2):
					...
					# The number of filters (argument name: num_outputs) is 96 in the original model . 
					# In the multiplexing code, the value can be reconfigured by the config argument.
					branch_2=slim.conv2d(net,selectdepth('Mixed_5c',48),[1,1],stride=1,scope='Conv2d_0a_1x1')
					############## end code changes##############
			...
			with tf.variable_scope(Logits):
				with tf.variable_scope(branch_0):
					branch_0=slim.conv2d(net,1000,[1,1],stride=1,scope='Conv2d_0c_1x1')
					Logits=branch_0
